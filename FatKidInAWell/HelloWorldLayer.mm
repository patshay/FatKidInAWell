...
//courtesy of ylwong updating to cocos2d v2
-(void) setupShader
{
...
-(CCSprite *)spriteWithColor:(ccColor4F)bgColor textureSize:(float)textureSize {
    ...
    //changes to setupshader and gl disable stuff from users.wfu.edu/ylwong showed how to update the code to new cocos3d version
    CC_NODE_DRAW_SETUP();
...
- (void)genBackground {
   ...
   // [self setupShader];
   //added to update to cocosv2, but now its not used?
...
-(id) init {
    if((self=[super init])) {
        ...
        //replaced to do terrain
        _terrain = [[[Terrain alloc] initWithWorld:_world withOffsetX:0] autorelease];
        _terrainR = [[[Terrain alloc] initWithWorld:_world withOffsetX:160] autorelease];
        
        [self addChild:_terrain z:1];
        [self addChild:_terrainR z:2];
        [self genBackground];
